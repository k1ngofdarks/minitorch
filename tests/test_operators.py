...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    # Stay away from saturation, where float rounding hides strict inequalities.
    assert 0.0 <= sigmoid(a) <= 1.0         # round problem with big numbers(
    assert_close(sigmoid(a) + sigmoid(-a), 1.0)
    assert_close(sigmoid(0.0), 0.5)
    assert lt(sigmoid(a), sigmoid(a + 1.0)) or eq(sigmoid(a), sigmoid(a + 1.0)) == 1.0
...
def test_other(a: float) -> None:
    """
    Check that relu(a) - relu(-a) equals a.
    """
    assert_close(add(relu(a), neg(relu(neg(a)))), a)
